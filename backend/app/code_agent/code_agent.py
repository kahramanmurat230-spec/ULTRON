...
class CodeAgent:
    ...
    def _apply_patch_checked(self,patch):
        ...
        for item in patch.get('files',[]):
            ...
            p.parent.mkdir(parents=True,exist_ok=True); p.write_text(item['content'],encoding='utf-8'); changed.append(item['path'])
    ...
    def self_repair(self, paths=None, apply=False, timeout=180, goal=None):
        """Analyze/propose when dry-run; approved apply uses bounded self-coding loop."""
        if apply:
    ...
    def self_coding_loop(self, paths=None, goal=None, approved=False, max_iterations=3, timeout=180):
        ...
        if not approved:return {'ok':False,'status':'APPROVAL_REQUIRED','iterations':[]}
        limit=max(1,min(int(max_iterations),self.MAX_REPAIR_ITERATIONS)); iterations=[]; feedback=None
        for index in range(1,limit+1):
            analysis=self.analyze(paths,goal=goal,feedback=feedback)
            try:patch=self.propose_patch(paths,goal=goal,feedback=feedback)
            except Exception as exc:return {'ok':False,'status':'PATCH_GENERATION_FAILED','iterations':iterations,'error':str(exc)}
            originals={}; entry={'iteration':index,'analysis':analysis,'patch_summary':patch.get('summary',''),'tests':None,'rolled_back':False}
            try:
                originals,_=self._apply_patch_checked(patch); test_result=self.test(timeout); entry['tests']=test_result
                if test_result['passed']:
                    entry['status']='PASS'; iterations.append(entry)
                    return {'ok':True,'status':'VERIFIED','iterations':iterations,'final':test_result}
                self._rollback(originals); entry['rolled_back']=True; entry['status']='FAIL_ROLLED_BACK'
                feedback=(test_result.get('stdout','')+'\n'+test_result.get('stderr',''))[-12000:]; iterations.append(entry)
            except Exception as exc:
                self._rollback(originals); entry['rolled_back']=True; entry['status']='ERROR_ROLLED_BACK'; entry['error']=str(exc); iterations.append(entry); feedback=str(exc)
