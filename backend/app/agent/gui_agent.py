...
class GUIAgent:
    MAX_ACTIONS = 8
    MUTATING_ACTIONS = {"click_text", "click", "double_click", "right_click", "type_text", "press", "hotkey", "scroll"}
    ...
    def execute(self, actions, approved=None):
        ...
        for action in actions:
            if not isinstance(action, GUIAction):
                return {"ok": False, "status": "REJECTED", "error": "Invalid GUI action"}
            if action.kind in self.MUTATING_ACTIONS and not approval:
