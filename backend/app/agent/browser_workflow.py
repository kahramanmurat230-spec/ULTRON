"""Deterministic browser workflow planner with explicit targets only."""
import re
...
def parse_browser_plan(text: str) -> list[dict]:
    ...
    for c in clauses:
        ...
        if m:
            steps.append({"label":"browser click selector","tool":"browser_click","args":{"selector":m.group(1).strip()}}); continue
        m = re.search(r"(?:role|rol)\s+([\w-]+)(?:\s+(?:adı|adi|name)\s+(.+?))?\s+(?:tıkla|tikla)$", c, re.I)
        if m:
            target="role="+m.group(1).strip()
            if m.group(2): target += "[name="+m.group(2).strip().strip("'\"")+"]"
            steps.append({"label":"browser click role","tool":"browser_click","args":{"selector":target}}); continue
        m = re.search(r"(?:text|metin)\s+['\"](.+?)['\"]\s+(?:tıkla|tikla)$", c, re.I)
        if m:
            steps.append({"label":"browser click text","tool":"browser_click","args":{"selector":"text="+m.group(1)}}); continue
        m = re.search(r"(?:selector|css)\s+['\"]?([^'\"]+)['\"]?\s+(?:alanına|alanina)\s+['\"](.+?)['\"]\s+yaz$", c, re.I)
        if m:
            steps.append({"label":"browser type target","tool":"browser_type","args":{"selector":m.group(1).strip(),"text":m.group(2)}}); continue
        if re.search(r"(?:formu|form)\s+(?:gönder|gonder|submit)$", low):
            steps.append({"label":"browser submit form","tool":"browser_submit","args":{}}); continue
        m = re.search(r"(?:sayfada|browser'da|browserda)\s+['\"](.+?)['\"]\s+(?:metnini|yazısını|yazisini)\s+doğrula$", c, re.I)
        if m:
            steps.append({"label":"browser verify text","tool":"browser_verify","args":{"text_contains":m.group(1)}}); continue
    return steps
