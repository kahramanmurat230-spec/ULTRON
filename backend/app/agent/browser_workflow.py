...
def parse_browser_plan(text: str) -> list[dict]:
    ...
    for c in clauses:
        ...
        m = re.search(
            rf"(?:siteye|adrese|url'ye|url ye|sayfaya)\s+{url_pattern}\s*(?:git|aç|ac)$",
            low,
            re.I,
        )
        if not m:
            m = re.search(rf"^{url_pattern}\s+(?:git|aç|ac)$", low, re.I)
        if not m:
            m = re.search(rf"^(?:git|aç|ac)\s+{url_pattern}$", low, re.I)
        if m:
