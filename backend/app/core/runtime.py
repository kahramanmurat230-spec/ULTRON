...
class UltronRuntime:
    ...
    def self_diagnostic(self):
        ...
        components["hardware"]={"status":hw_status,"cpu_percent":cpu,"ram_percent":ram,"disk_percent":disk,"gpus":stats.get("gpus",[]) if isinstance(stats,dict) else []}
        statuses=[v.get("status") for v in components.values()]; overall="ERROR" if "ERROR" in statuses else ("WARN" if "WARN" in statuses else "OK")
        warnings=[k for k,v in components.items() if v.get("status")=="WARN"]; errors=[k for k,v in components.items() if v.get("status")=="ERROR"]
    ...
    def _register_tools(self):
        ...
        def _copy_path(source,destination):
            src=self.sandbox.validate_read(source); dst_dir=self.sandbox.validate_write(destination); return copy_path(src,dst_dir/Path(src).name)
        def _move_path(source,destination):
            src=self.sandbox.validate_read(source); dst_dir=self.sandbox.validate_write(destination); return move_path(src,dst_dir/Path(src).name)
        def _rename_path(path,new_name):
            src=self.sandbox.validate_write(path); self.sandbox.validate_write(src.with_name(Path(new_name).name)); return rename_path(src,new_name)
        def _create_folder(path): return create_folder(self.sandbox.validate_write(path))
        def _delete_path(path): return delete_path(self.sandbox.validate_write(path))
        ...
        reg.register('browser_click',lambda selector:br().click(selector),'Sayfadaki elemente tıklar; onay gerekir.',{'type':'object','properties':{'selector':{'type':'string'}},'required':['selector']},dangerous=True)
        ...
        reg.register('process_list',lambda sort='cpu',limit=30,name=None:process_list(sort,limit,name),'Süreç listeler.',{'type':'object','properties':{'sort':{'type':'string'},'limit':{'type':'number'},'name':{'type':'string'}},'required':[]})
        ...
        reg.register('system_settings_view',lambda:system_settings_view(),'Sistem ayarlarını salt-okunur listeler.',{'type':'object','properties':{},'required':[]})
        ...
        reg.register('weather_forecast',lambda city='Mersin':self._weather.forecast(city),'Yarın tahmini.',{'type':'object','properties':{'city':{'type':'string'}},'required':['city']})
        reg.register('calendar_events',lambda days=7:self._calendar.events(days),'Takvim etkinlikleri.',{'type':'object','properties':{'days':{'type':'number'}},'required':[]})
        ...
        reg.register('email_search',lambda query,limit=10:self._email.search(query,limit=limit),'E-posta arar.',{'type':'object','properties':{'query':{'type':'string'},'limit':{'type':'number'}},'required':['query']})
        ...
        reg.register('skills_list',lambda:self.skills.list(),'Yüklü skilleri listeler.',{'type':'object','properties':{},'required':[]})
        ...
        reg.register('read_text',lambda path:sandboxed_read_text(self.sandbox,path),'Metin/kod dosyası okur.',{'type':'object','properties':{'path':{'type':'string'}},'required':['path']})
        ...
        reg.register('image_analyze',lambda path:analyze_image(path),'Görüntüyü LLM olmadan analiz eder.',{'type':'object','properties':{'path':{'type':'string'}},'required':['path']})
        reg.register('image_diff',lambda path_a,path_b:diff_images(path_a,path_b),'İki kare arasındaki değişimi ölçer.',{'type':'object','properties':{'path_a':{'type':'string'},'path_b':{'type':'string'}},'required':['path_a','path_b']})
        reg.register('ocr_elements',lambda path:_ocr_el(path),'OCR metin + kutu koordinatları döner.',{'type':'object','properties':{'path':{'type':'string'}},'required':['path']})
        ...
        reg.register('self_repair',lambda paths=None,apply=False:self.code_agent.self_repair(paths,apply=apply and self.approved),'Kodu analiz eder ve onaylı uygular.',{'type':'object','properties':{'paths':{'type':'array','items':{'type':'string'}},'apply':{'type':'boolean'}},'required':[]},dangerous=True)
        reg.register('write_text',lambda path,content:sandboxed_write_text(self.sandbox,path,content),'Metin dosyası yazar; onay gerekir.',{'type':'object','properties':{'path':{'type':'string'},'content':{'type':'string'}},'required':['path','content']},dangerous=True)
        ...
        reg.register('gui_locate_and_click',self.gui.locate_and_click,'Ekran şablonunu bulup tıklar; onay gerekir.',{'type':'object','properties':{'image':{'type':'string'},'confidence':{'type':'number'}},'required':['image']},dangerous=True)
        reg.register('read_screen_elements',lambda:self.gui.read_screen_elements(),'Ekranı OCR ile okur.',{'type':'object','properties':{},'required':[]})
        ...
        reg.register('apply_code_patch',lambda patch:self.code_agent.apply_patch(patch),'Kod patchini uygular; onay gerekir.',{'type':'object','properties':{'patch':{'type':'object'}},'required':['patch']},dangerous=True)
        reg.register('code_intel_analyze',lambda target='all':self.code_intel.analyze(target),'Projeyi statik analiz eder.',{'type':'object','properties':{'target':{'type':'string'}},'required':[]})
