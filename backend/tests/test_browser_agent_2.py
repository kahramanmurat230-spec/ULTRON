...
def test_browser_2_risk_and_explicit_workflow():
    ...
    steps=parse_browser_plan("https://example.com aç, sonra role button adı Gönder tıkla, sonra formu gönder, sonra sayfada 'Tamamlandı' metnini doğrula")
    assert [s["tool"] for s in steps]==["browser_navigate","browser_click","browser_submit","browser_verify"]
...
def test_browser_submit_requires_unambiguous_form_without_selector():
    ...
    agent=BrowserAgent(settings={"browser":{"timeout_seconds":1}},engine_factory=lambda:(Browser(),PW()))
    try:
        try: agent.submit(); assert False,"ambiguous submit must fail"
        except ValueError as exc: assert "exactly one form" in str(exc)
    finally: agent.close()
