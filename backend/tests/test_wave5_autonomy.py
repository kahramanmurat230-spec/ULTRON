...
def make_safety(tmp, **pol):
    return AutonomySafety(db_path=os.path.join(tmp, "s.db"),
                          approval_policy=pol)
...
def test_safety_high_risk_needs_user_approval():
    with tempfile.TemporaryDirectory() as d:
        ...
        r2 = s.evaluate("temizlik", "delete all temp files",
                        reversibility="irreversible", user_approved=True)
...
def test_safety_medium_policy_gate():
    with tempfile.TemporaryDirectory() as d:
        ...
        r2 = s.evaluate("dağıt", "write config file",
                        reversibility="partially-reversible",
                        policy_allows=False)
...
def test_dry_run_previews_and_never_executes():
    with tempfile.TemporaryDirectory() as d:
        ...
        r = sim.dry_run("güncelle", [
            {"name": "write file.txt", "type": "write", "path": "file.txt",
             "content": "yeni içerik daha uzun"},
        ...
        # GERÇEK dosya DEĞİŞMEDİ (dry-run kanıtı)
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        w = sim.what_if("deploy", {
            "hızlı": [{"name": "shell push", "type": "shell",
                       "command": "git push"}],
            "güvenli": [{"name": "read check", "type": "read"}],
        })
...
def ok_exec(action):
    return {"ok": True, "output": f"done:{action.get('name')}",
            "cost": 0.01, "tokens": 10}
...
def test_loop_completes_plan_with_checkpoints():
    with tempfile.TemporaryDirectory() as d:
        ...
        r = loop.run("hedef", [
            ...
            verifier=lambda res, goal: res.get("ok"))
...
def test_loop_budget_iteration_limit():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert r["stop_reason"] == "budget:iterations"
...
def test_loop_budget_time_limit():
    with tempfile.TemporaryDirectory() as d:
        ...
        r = loop.run("yavaş iş", [{"name": "a"}, {"name": "b"},
                                  {"name": "c"}, {"name": "d"}], slow,
                     budgets=Budgets(max_iterations=10, max_seconds=0.8))
        assert r["status"] == "STOPPED_BUDGET"
...
def test_loop_cancel_stops_immediately():
    with tempfile.TemporaryDirectory() as d:
        ...
        def cancelling_exec(a):
            loop.cancel(1)  # run_id ilk çalıştırmada 1
            return {"ok": True, "output": "x"}
        r = loop.run("iptal", [{"name": "a"}, {"name": "b"}],
                     cancelling_exec)
...
def test_loop_verify_fail_without_replan_stops():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert r["status"] == "VERIFY_FAILED" and r["iterations"] == 1
...
def test_loop_replan_then_success():
    with tempfile.TemporaryDirectory() as d:
        ...
        def replan(failed, result):
            return [{"name": "dayanıklı"}]

        r = loop.run("dayanıklılık", [{"name": "kırılgan"}],
                     exec_once_fails,
                     verifier=lambda res, goal: res.get("ok"),
                     replan_fn=replan)
        assert r["status"] == "COMPLETED" and r["replans"] == 1
...
def test_loop_safety_deny_stops_run():
    with tempfile.TemporaryDirectory() as d:
        ...
        r = loop.run("tehlikeli", [{"name": "delete everything"}], ok_exec,
                     policy_allows=False)
        assert r["status"] == "STOPPED_SAFETY"
...
def test_loop_goal_completion_requires_evidence_path():
    with tempfile.TemporaryDirectory() as d:
        ...
        r = loop.run("otonom hedef", [{"name": "adım"}],
                     lambda a: {"ok": True, "output": "çıktı: 3 kayıt"},
                     verifier=lambda res, goal: True, goal_id=gid)
...
def test_run_status_persisted():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert st["status"] == "COMPLETED" and st["iterations"] == 1
        assert st["result"]["budget"]["limits"]["iterations"] > 0
