...
def test_risk_classification():
    ...
    assert browser_action_risk("navigate") == "SAFE"; assert browser_action_risk("screenshot") == "SAFE"
    assert browser_action_risk("read_text") == "SAFE"; assert browser_action_risk("verify") == "SAFE"
    assert browser_action_risk("click") == "MEDIUM"; assert browser_action_risk("type") == "MEDIUM"
    assert browser_action_risk("select") == "MEDIUM"; assert browser_action_risk("submit") == "MEDIUM"
    assert browser_action_risk("hack_the_planet") == "UNKNOWN"
    for a in ("navigate", "read_text", "read_dom", "find_elements"): assert a not in STATE_CHANGING


def test_normalize_url():
    assert normalize_url("example.com") == "https://example.com"; assert normalize_url("https://a.b/c?d=1") == "https://a.b/c?d=1"
    assert normalize_url("http://x.y") == "http://x.y"; assert normalize_url("file:///tmp/page.html") == "file:///tmp/page.html"; assert normalize_url("about:blank") == "about:blank"
...
class FakePage:
    def __init__(self, engine):
        self.engine=engine; self.url=engine.home; self.title_text="Test"; self.body="hello world"; self.selectors={"#btn":1,".item":3,"#missing":0}; self.filled={}; self.clicked=[]
    def title(self):
...
class FakeLocator:
    ...
    def inner_text(self,timeout=None): return self.page.body if self.sel=="body" else "text-of-"+self.sel
    ...
    def click(self,timeout=None):
        if self.page.selectors.get(self.sel,0)==0: raise RuntimeError("element not found")
        self.page.clicked.append(self.sel); self.page.body += " clicked"
    def fill(self,text,timeout=None):
        if self.page.selectors.get(self.sel,0)==0: raise RuntimeError("element not found")
        self.page.filled[self.sel]=text
...
def test_browser_agent_lifecycle_navigate_click_type_verify(tmp_path):
    ...
    try:
        res=agent.navigate("example.com"); assert res["url"]=="https://example.com"; assert res["title"]=="Page: https://example.com"; page=agent._pages[0]
        assert agent.read_text()["text"]=="hello world"; assert agent.click("#btn")["clicked"]=="#btn"; assert page.clicked==["#btn"]
        assert agent.type("#btn","merhaba",press_enter=True)["typed"]==7; assert page.filled["#btn"]=="merhaba"; assert len(agent.find_elements(".item",limit=2))==2
        shot=agent.screenshot(str(tmp_path/"s.png")); assert shot["bytes"]>0; v=agent.verify(url_contains="example",text_contains="clicked"); assert v["ok"] and all(v["checks"].values()); assert agent.verify(text_contains="asla-yok")["ok"] is False
    finally: agent.close()
    ...
    try:
        ...
        with sync_playwright() as p:
            for kw in ({"headless":True},{"headless":True,"channel":"chrome"},{"headless":True,"channel":"msedge"}):
                try: b=p.chromium.launch(**kw); b.close(); return True
...
def test_real_chromium_e2e_local_page(tmp_path):
    html=tmp_path/"page.html"; html.write_text("<html><head><title>ULTRON Test Sayfası</title></head><body><h1 id='hdr'>Merhaba Tarayıcı</h1><form id='f'><input id='q' name='q'><button id='go' type='submit'>Git</button></form><div id='result'>Tamamlandı</div></body></html>",encoding="utf-8")
    agent=BrowserAgent(settings={"browser":{"headless":True,"timeout_seconds":10}})
    try:
        assert "ULTRON Test" in agent.navigate(html.as_uri())["title"]; assert agent.verify(title_contains="ULTRON",text_contains="Merhaba")["ok"]; assert agent.type("#q","ultron sorgusu")["typed"]>0
        assert agent.find_elements(role="button",name="Git")[0]["text"]=="Git"; assert agent.click("role=button[name=Git]")["clicked"]=="role=button[name=Git]"; assert "Merhaba" in agent.read_dom("#hdr")["html"]
        assert agent.submit("#f")["method"]=="requestSubmit"; assert agent.verify(text_contains="Tamamlandı")["ok"]; assert Path(agent.screenshot()["path"]).stat().st_size>0
    finally: agent.close()
